...
function getTablePks(PDO $pdo, string $table): array {
  try {
    $cols = $pdo->query("PRAGMA table_info(" . qi($table) . ")")->fetchAll();
    $pks = [];
    foreach ($cols as $c) {
      if (!empty($c['pk'])) {
        $pks[] = $c['name'];
...
function isBinaryData(mixed $val): bool {
  return is_string($val) && (preg_match('/[^\x20-\x7E\t\r\n]/', $val) === 1 || str_contains($val, "\0"));
}
...
function streamSqlDump(PDO $pdo): void {
  echo "-- DBLiteAdmin SQL Dump\nPRAGMA foreign_keys=OFF;\nBEGIN TRANSACTION;\n";
  $tblStmt = $pdo->query("SELECT name, sql FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'");
  while ($tbl = $tblStmt->fetch()) {
    echo "\nDROP TABLE IF EXISTS " . qi($tbl['name']) . ";\n{$tbl['sql']};\n";
    $rows = $pdo->query("SELECT * FROM " . qi($tbl['name']));
    while ($r = $rows->fetch()) {
      $cols = array_map('qi', array_keys($r));
      $vals = [];
      foreach ($r as $val) {
        if ($val === null) {
          $vals[] = 'NULL';
        } elseif (isBinaryData($val)) {
          $vals[] = "X'" . bin2hex($val) . "'";
        } else {
          $vals[] = $pdo->quote((string)$val);
        }
      }
      echo "INSERT INTO " . qi($tbl['name']) . " (" . implode(',', $cols) . ") VALUES (" . implode(',', $vals) . ");\n";
    }
...
if (php_sapi_name() === 'cli-server') {
  ...
  if ($urlPath !== '/' && file_exists($filePath) && !is_dir($filePath) && realpath($filePath) !== __FILE__) {
    return false;
  }
}
...
if (php_sapi_name() === 'cli') {
  global $argv;
  if (isset($argv[1]) && in_array($argv[1], ['serve', 'server', '-S'], true)) {
    $bind = $argv[2] ?? '127.0.0.1:8000';
    if (!str_contains($bind, ':')) {
      $bind = '127.0.0.1:' . $bind;
    }
    fwrite(STDOUT, APP_NAME . " server running at http://{$bind}\nPress Ctrl+C to terminate.\n\n");
    passthru(escapeshellcmd(PHP_BINARY) . " -S " . escapeshellarg($bind) . " " . escapeshellarg(__FILE__));
    exit(0);
  }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($act === 'update_row' && $table && $pdo) {
    $rowKey = $_POST['__rowid__'] ?? '';
    $sets = []; $p = [];
    foreach ($_POST['col'] ?? [] as $k => $val) {
      if (preg_match('/^\[BLOB \d+ bytes\]$/', (string)$val)) {
        continue;
      }
      $sets[] = qi($k) . " = " . ($val === '' ? "NULL" : "?");
      if ($val !== '') $p[] = $val;
    ...
    if ($rowKey !== '' && $sets) {
      if (str_starts_with($rowKey, 'pk:')) {
        $keyPairs = json_decode(base64_decode(substr($rowKey, 3)), true) ?: [];
        $wherePks = [];
        foreach ($keyPairs as $pkCol => $pkVal) {
          $wherePks[] = qi($pkCol) . ' = ?';
          $p[] = $pkVal;
        }
        if ($wherePks) {
          $pdo->prepare("UPDATE " . qi($table) . " SET " . implode(', ', $sets) . " WHERE " . implode(' AND ', $wherePks))->execute($p);
        }
      } else {
        $p[] = $rowKey;
  ...
  if ($act === 'delete_row' && $table && $pdo && isset($_POST['rowid'])) {
    ...
    if (str_starts_with($rowKey, 'pk:')) {
      ...
      if ($wherePks) {
        $pdo->prepare("DELETE FROM " . qi($table) . " WHERE " . implode(' AND ', $wherePks))->execute($p);
      }
    } else {
      $pdo->prepare("DELETE FROM " . qi($table) . " WHERE rowid = ?")->execute([$rowKey]);
    }
    header("Location: ?db=" . urlencode($currentDb) . "&tab=browse&table=" . urlencode($table));
    exit;
  }
  if ($act === 'bulk_delete' && $table && $pdo && !empty($_POST['selected_rowids'])) {
    ...
    foreach ($ids as $key) {
      if ($key === '') continue;
      if (str_starts_with($key, 'pk:')) {
        $keyPairs = json_decode(base64_decode(substr($key, 3)), true) ?: [];
        $wherePks = []; $p = [];
        foreach ($keyPairs as $pkCol => $pkVal) {
          $wherePks[] = qi($pkCol) . ' = ?';
          $p[] = $pkVal;
        ...
        $pdo->prepare("DELETE FROM " . qi($table) . " WHERE rowid = ?")->execute([$key]);
      }
    }
    $pdo->commit();
    header("Location: ?db=" . urlencode($currentDb) . "&tab=browse&table=" . urlencode($table));
  ...
  if ($act === 'set_display_prefs') {
    $lim = max(10, min(2000, (int)($_POST['default_limit'] ?? 25)));
    $_SESSION['default_limit'] = $lim;
    header("Location: ?db=" . urlencode($currentDb) . "&tab=settings&msg=Preferences+saved");
    exit;
  }
...
if ($tab === 'blob_raw' && $pdo && !empty($_GET['table']) && !empty($_GET['col']) && isset($_GET['key'])) {
  $bTable = $_GET['table'];
  $bCol = $_GET['col'];
  $bKey = (string)$_GET['key'];
  if (str_starts_with($bKey, 'pk:')) {
    $pks = json_decode(base64_decode(substr($bKey, 3)), true) ?: [];
    $w = []; $p = [];
    foreach ($pks as $pkCol => $pkVal) {
      $w[] = qi($pkCol) . ' = ?';
      $p[] = $pkVal;
    }
    $st = $pdo->prepare("SELECT " . qi($bCol) . " FROM " . qi($bTable) . " WHERE " . implode(' AND ', $w) . " LIMIT 1");
    $st->execute($p);
  } else {
    $st = $pdo->prepare("SELECT " . qi($bCol) . " FROM " . qi($bTable) . " WHERE rowid = ? LIMIT 1");
    $st->execute([$bKey]);
  }
  $blobData = $st->fetchColumn();
  if ($blobData === false) {
    http_response_code(404);
    exit('Not found');
  }
  $strData = (string)$blobData;
  $mime = 'application/octet-stream';
  if (str_starts_with($strData, 'data:image/') && str_contains($strData, ';base64,')) {
    [$hdr, $b64] = explode(';base64,', $strData, 2);
    $mime = substr($hdr, 5);
    $strData = base64_decode($b64);
  } elseif (strlen($strData) > 60 && preg_match('/^(iVBORw0KGgo|\/9j\/|R0lGOD|UklGR)/', trim($strData))) {
    $t = trim($strData);
    $mime = 'image/png';
    if (str_starts_with($t, '/9j/')) $mime = 'image/jpeg';
    elseif (str_starts_with($t, 'R0lGOD')) $mime = 'image/gif';
    elseif (str_starts_with($t, 'UklGR')) $mime = 'image/webp';
    $strData = base64_decode($t);
  } elseif (str_starts_with($strData, "\x89PNG")) $mime = 'image/png';
  elseif (str_starts_with($strData, "\xFF\xD8\xFF")) $mime = 'image/jpeg';
  elseif (str_starts_with($strData, "GIF8")) $mime = 'image/gif';
  elseif (str_starts_with($strData, "RIFF") && substr($strData, 8, 4) === "WEBP") $mime = 'image/webp';
  elseif (str_starts_with($strData, "%PDF")) $mime = 'application/pdf';
  ...
  if (isset($_GET['dl'])) {
    header('Content-Disposition: attachment; filename="' . preg_replace('/[^a-zA-Z0-9_\-]/', '_', $bCol) . '.bin"');
  } else {
...
function renderView($tab, $table, $pdo, $currentDb, $tables, string $sqliteVer, string $dbSize, array $dbs) {
  ...
  <div class="view-shell">
    ...
    <?php elseif ($tab === 'browse' && $table): ?>
      <?php
      $defaultLimit = (int)($_SESSION['default_limit'] ?? 25);
      $page = max(1, (int)($_GET['p'] ?? 1));
      $limit = max(10, min(2000, (int)($_GET['limit'] ?? $defaultLimit)));
      $q = trim($_GET['q'] ?? '');
      $sort = trim($_GET['sort'] ?? '');
      ...
      $st->execute($p);
      $rows = $st->fetchAll();
      $tablePks = getTablePks($pdo, $table);
      $canManageRows = $hasRowId || !empty($tablePks);
      ?>
      <div class="headline">
        <h1 class="title"><?= htmlspecialchars($table) ?></h1>
        <p class="subtitle"><?= $total ?> total records<?= !$hasRowId ? ' (WITHOUT ROWID)' : '' ?></p>
      </div>
      ...
      <div class="row between" style="gap:.75rem;margin-bottom:1rem;flex-wrap:wrap">
        <form method="get" class="search-form" style="margin-bottom:0;flex:1;min-width:200px" onsubmit="event.preventDefault(); nav('?db=<?= urlencode($currentDb) ?>&tab=browse&table=<?= urlencode($table) ?>&limit=<?= $limit ?>&q=' + encodeURIComponent(this.q.value));">
          <div class="search-input-wrap">
            <span class="search-icon"><?= md_icon('search', 18) ?></span>
            <input type="search" name="q" value="<?= htmlspecialchars($q) ?>" placeholder="Search in <?= htmlspecialchars($table) ?>..." class="input search-input">
          </div>
        </form>
        <div class="row gap-sm" style="flex-shrink:0">
          <span class="muted" style="font-size:.78rem">Per page:</span>
          <select class="input-limit" onchange="changeRowLimit(this.value)">
      ...
      <?php
      $tablePks = getTablePks($pdo, $table);
      $canManage = $hasRowId || !empty($tablePks);
      $tableColInfo = $pdo->query("PRAGMA table_info(" . qi($table) . ")")->fetchAll();
      $colTypeMap = [];
      foreach ($tableColInfo as $ci) {
        $colTypeMap[$ci['name']] = strtoupper((string)($ci['type'] ?? ''));
      }

      $compactRows = [];
      foreach ($rows as $r) {
        if ($hasRowId) {
          $rowKey = (string)$r['__rowid__'];
        } else {
          $pkVals = [];
          foreach ($tablePks as $pk) $pkVals[$pk] = $r[$pk] ?? '';
          $rowKey = 'pk:' . base64_encode(json_encode($pkVals));
        }

        $rowTuple = [];
        foreach ($cols as $c) {
          $val = $r[$c] ?? null;
          $cDeclared = $colTypeMap[$c] ?? '';
          $isBlobCol = str_contains($cDeclared, 'BLOB') || str_contains($cDeclared, 'BIN');

          if ($val === null) {
            $rowTuple[] = null;
          } elseif (isBinaryData($val)) {
            $bLen = strlen($val);
            $isImg = (str_starts_with($val, "\x89PNG") || str_starts_with($val, "\xFF\xD8\xFF") || str_starts_with($val, "GIF8") || (str_starts_with($val, "RIFF") && substr($val, 8, 4) === "WEBP"));
            $hexSample = bin2hex(substr($val, 0, 256));
            $rowTuple[] = ['__b' => 1, 'len' => $bLen, 'img' => $isImg ? 1 : 0, 'hex' => $hexSample];
          } elseif (is_string($val) && (str_starts_with($val, 'data:image/') || (strlen($val) > 60 && preg_match('/^(iVBORw0KGgo|\/9j\/|R0lGOD|UklGR)/', trim($val))))) {
            $tVal = trim($val);
            $bLen = strlen($tVal);
            $m = 'image/png';
            if (str_starts_with($tVal, '/9j/')) $m = 'image/jpeg';
            elseif (str_starts_with($tVal, 'R0lGOD')) $m = 'image/gif';
            elseif (str_starts_with($tVal, 'UklGR')) $m = 'image/webp';
            $uri = str_starts_with($tVal, 'data:image/') ? $tVal : ("data:{$m};base64," . $tVal);
            $rowTuple[] = ['__b' => 1, 'len' => $bLen, 'img' => 1, 'uri' => $uri];
          } elseif ($isBlobCol) {
            $tStr = (string)$val;
            $bLen = strlen($tStr);
            $hexSample = bin2hex(substr($tStr, 0, 256));
            $rowTuple[] = ['__b' => 1, 'len' => $bLen, 'img' => 0, 'hex' => $hexSample];
          } elseif (is_string($val) && strlen(trim($val)) > 80 && preg_match('/^[A-Za-z0-9+\/=\r\n]+$/', trim($val)) && base64_decode(trim($val), true) !== false) {
            $decoded = base64_decode(trim($val));
            $bLen = strlen($decoded);
            $isImg = (str_starts_with($decoded, "\x89PNG") || str_starts_with($decoded, "\xFF\xD8\xFF") || str_starts_with($decoded, "GIF8") || (str_starts_with($decoded, "RIFF") && substr($decoded, 8, 4) === "WEBP"));
            $hexSample = bin2hex(substr($decoded, 0, 256));
            $rowTuple[] = ['__b' => 1, 'len' => $bLen, 'img' => $isImg ? 1 : 0, 'hex' => $hexSample];
          } else {
            $rowTuple[] = (string)$val;
          }
        }
        $compactRows[] = [$rowKey, $rowTuple];
      }
      ?>

      <div class="tbl-box" id="tbl-scroller" data-db="<?= htmlspecialchars($currentDb) ?>" data-table="<?= htmlspecialchars($table) ?>" onscroll="handleTableScroll()">
        <table class="tbl">
          <thead>
            <tr>
              <?php if ($canManage): ?>
                <th style="width:40px;min-width:40px;max-width:40px;text-align:center"><input type="checkbox" id="chk-all" onchange="toggleSelectAll(this)"></th>
                <th style="width:74px;min-width:74px;max-width:74px">Actions</th>
              <?php endif; ?>
              <?php foreach ($cols as $c): ?>
                <th>
                  <a href="?db=<?= urlencode($currentDb) ?>&tab=browse&table=<?= urlencode($table) ?>&limit=<?= $limit ?>&q=<?= urlencode($q) ?>&sort=<?= urlencode($c) ?>&dir=<?= ($sort === $c && $dir === 'ASC') ? 'DESC' : 'ASC' ?>" class="tbl-sort" data-nav>
                    <?= htmlspecialchars($c) ?>
          ...
          <tbody id="tbl-vbody">
            <tr id="v-pad-top" style="height:0px;border:none"><td colspan="100%" style="padding:0;height:0;border:none"></td></tr>
            <tr id="v-pad-bottom" style="height:0px;border:none"><td colspan="100%" style="padding:0;height:0;border:none"></td></tr>
          </tbody>
        </table>
        <?php if (empty($rows)): ?>
          <div class="center muted" style="padding:2.5rem 1rem">No records found</div>
        <?php endif; ?>
      </div>

      <script id="tbl-cols-json" type="application/json"><?= json_encode($cols, JSON_HEX_TAG | JSON_HEX_AMP) ?></script>
      <script id="tbl-raw-json" type="application/json"><?= json_encode($compactRows, JSON_INVALID_UTF8_SUBSTITUTE | JSON_HEX_TAG | JSON_HEX_AMP) ?></script>

      <?php if ($total > $limit): ?>
        <div class="between pagination" style="margin-top:1rem">
          <span class="subtitle">Page <?= $page ?> of <?= (int)ceil($total / $limit) ?></span>
          <div class="row gap-sm">
            <?php if ($page > 1): ?><a href="?db=<?= urlencode($currentDb) ?>&tab=browse&table=<?= urlencode($table) ?>&limit=<?= $limit ?>&q=<?= urlencode($q) ?>&sort=<?= urlencode($sort) ?>&dir=<?= $dir ?>&p=<?= $page - 1 ?>" class="btn btn-tonal" data-nav>« Prev</a><?php endif; ?>
            <?php if ($offset + $limit < $total): ?><a href="?db=<?= urlencode($currentDb) ?>&tab=browse&table=<?= urlencode($table) ?>&limit=<?= $limit ?>&q=<?= urlencode($q) ?>&sort=<?= urlencode($sort) ?>&dir=<?= $dir ?>&p=<?= $page + 1 ?>" class="btn btn-tonal" data-nav>Next »</a><?php endif; ?>
          </div>
        </div>
      <?php endif; ?>
    ...
    <?php elseif ($tab === 'relations'): ?>
      ...
      <div class="erd-viewport" id="erd-viewport">
        <div class="erd-controls">
          ...
          <div class="erd-controls-row" style="margin-top:4px;border-top:1px solid var(--border);padding-top:4px">
            <button type="button" class="erd-ctrl-btn" onclick="zoomErd(0.15)" title="Zoom In">+</button>
            <button type="button" class="erd-ctrl-btn" id="erd-zoom-btn" onclick="resetErdZoom()" title="Reset Zoom" style="width:auto;padding:0 5px;font-size:.68rem">100%</button>
          ...
          <tbody>
            <?php foreach ($pdo->query("PRAGMA table_info(" . qi($table) . ")") as $c): ?>
              <tr>
                ...
                <td style="background:transparent !important"><span style="color:var(--out);font-family:monospace;font-size:.82rem;background:transparent !important;border:none;padding:0"><?= htmlspecialchars($c['type'] ?: 'ANY') ?></span></td>
    ...
    <?php elseif ($tab === 'settings'): ?>
      ...
      <div class="card">
        <h3 style="margin:0 0 .8rem">Display Preferences</h3>
        <p class="muted" style="margin-bottom:.8rem">Set the default number of rows displayed per page when browsing tables.</p>
        <form method="post" class="row gap-sm" style="max-width:400px">
          <input type="hidden" name="csrf" value="<?= $csrf ?>">
          <input type="hidden" name="action" value="set_display_prefs">
          <input type="number" name="default_limit" min="10" max="2000" step="5" value="<?= htmlspecialchars((string)($_SESSION['default_limit'] ?? 25)) ?>" class="input" style="width:140px" required>
          <button class="btn btn-prim" style="white-space:nowrap">Save Rows</button>
        </form>
      </div>
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
      ...
      .tbl-box { width: 100%; max-width: 100%; max-height: calc(100dvh - 280px); overflow-x: auto; overflow-y: auto; border-radius: 16px; border: 1px solid var(--ch); -webkit-overflow-scrolling: touch; overscroll-behavior: contain; position: relative; }
      .tbl { width: max-content; min-width: 100%; border-collapse: separate; border-spacing: 0; font-size: .83rem; white-space: nowrap; table-layout: fixed; }
      .tbl th, .tbl td { padding: 0 .85rem; text-align: left; height: 44px; max-height: 44px; line-height: 44px; vertical-align: middle; box-sizing: border-box; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; min-width: 110px; max-width: 320px; }
      .tbl th { background: var(--ch); color: var(--out); font-weight: 600; position: sticky; top: 0; z-index: 5; border-bottom: 2px solid var(--out-var); }
      .tbl td { border-bottom: 1px solid var(--ch); }
      .tbl tbody tr:hover td { background: rgba(125,125,125,.04); }
      .tbl-sort { color: inherit; text-decoration: none; display: inline-flex; align-items: center; gap: 4px; }
      .tab-btn { background: none; border: none; padding: 0; cursor: pointer; font-family: inherit; }
      .tab-btn .pill { color: var(--p); }
      .tab-btn:active .pill { background: var(--pc); color: var(--opc); }
      .blob-thumb { height: 26px; width: 26px; border-radius: 4px; object-fit: cover; border: 1px solid var(--border); vertical-align: middle; cursor: pointer; }
      .input-limit { background: var(--ch); color: var(--os); border: 1px solid var(--out-var); border-radius: 10px; padding: 4px 8px; font-size: .78rem; font-family: inherit; outline: none; cursor: pointer; }
      .sort-arr { font-size: .65rem; color: var(--p); }
      .blob-thumb { height: 28px; width: 28px; border-radius: 6px; object-fit: cover; border: 1px solid var(--border); cursor: pointer; flex-shrink: 0; }
      .bnav-add-btn { background: none; border: none; padding: 0; cursor: pointer; display: flex; align-items: center; justify-content: center; }
      .bnav-add-pill { width: 44px; height: 32px; border-radius: 99px; background: var(--pc); color: var(--opc); display: flex; align-items: center; justify-content: center; transition: transform .1s; }
      .bnav-add-btn:active .bnav-add-pill { transform: scale(0.92); }
      ...
      @media(min-width:768px) {
        ...
        .main { padding: 2.5rem; }
        .bnav-add-btn { order: -1; margin-bottom: .6rem; }
        .bnav-add-pill { width: 48px; height: 48px; border-radius: 16px; box-shadow: 0 2px 8px rgba(0,0,0,.2); }
  ...
  <body>
    <div class="app">
      <header class="topbar">
        ...
        <div class="row gap-sm">
          ...
          <?php if ($currentDb): ?>
            <a href="?db=<?= urlencode($currentDb) ?>&tab=settings" id="topbar-db-link" class="chip" data-nav><?= htmlspecialchars($currentDb) ?></a>
          <?php endif; ?>
        </div>
      </header>
      ...
      <nav class="bnav">
        ...
        <?php if ($pdo): ?>
          <button type="button" class="tab tab-btn" onclick="openSheet()" aria-label="Create / Add">
            <div class="pill"><?= md_icon('add', 20) ?></div><span>New</span>
          </button>
        <?php endif; ?>
    ...
    <?php if ($pdo): ?>
      ...
      <div class="sheet-bg" id="blob-sheet" onclick="closeBlobSheet(event)">
        <div class="sheet" onclick="event.stopPropagation()">
          <div class="drag-bar"></div>
          <div class="between" style="margin-bottom:1rem">
            <h2 id="blob-title" style="margin:0;font-size:1.25rem">BLOB Viewer</h2>
            <a id="blob-dl-btn" href="#" class="btn btn-prim" style="min-height:34px;padding:0 14px;font-size:.78rem" download>Download File</a>
          </div>
          <div id="blob-content" style="max-height:55dvh;overflow:auto;margin-bottom:1rem;border-radius:12px"></div>
          <div class="row" style="justify-content:flex-end">
            <button type="button" class="btn btn-tonal" onclick="closeBlobSheet()">Close</button>
          </div>
        </div>
      </div>
    <?php endif; ?>
    ...
    <script>
      ...
      function updateDynamicNavLinks(targetUrl) {
        const parsed = new URL(targetUrl, location.origin);
        const curDb = parsed.searchParams.get('db') || '';
        const curTbl = parsed.searchParams.get('table') || '';

        document.querySelectorAll('.bnav a.tab').forEach(a => {
          const aUrl = new URL(a.href, location.origin);
          const tabName = aUrl.searchParams.get('tab') || 'tables';
          const p = new URLSearchParams();
          if (curDb) p.set('db', curDb);
          p.set('tab', tabName);
          if (curTbl && (tabName === 'browse' || tabName === 'structure')) {
            p.set('table', curTbl);
          }
          a.href = '?' + p.toString();
        });

        const topLink = document.getElementById('topbar-db-link');
        if (topLink && curDb) {
          topLink.href = `?db=${encodeURIComponent(curDb)}&tab=settings`;
          topLink.textContent = curDb;
        }
      }

      async function nav(url, push = true) {
        if (typeof closeBlobSheet === 'function') closeBlobSheet();
        if (typeof closeEditSheet === 'function') closeEditSheet();
        if (typeof closeSheet === 'function') closeSheet();
        if (typeof closeRelationSheet === 'function') closeRelationSheet();
        ...
        const apply = async () => {
          ...
          updateDynamicNavLinks(url);
          if (push) history.pushState({ url }, '', url);
          if (tab === 'relations') {
            requestAnimationFrame(() => initErd());
          }
          if (tab === 'browse') {
            initVirtualTable();
            requestAnimationFrame(() => initVirtualTable());
          }
        };
      ...
      window.addEventListener('popstate', e => { nav(e.state?.url || location.href, false); });
      ...
      function openBlobSheet() { document.getElementById('blob-sheet')?.classList.add('active'); }
      function closeBlobSheet(e) { if (!e || e.target?.id === 'blob-sheet' || !e.target) document.getElementById('blob-sheet')?.classList.remove('active'); }

      function viewBlobDetail(colName, rowKey, isImg, rawLen, hexSample, isDirect = false) {
        const titleEl = document.getElementById('blob-title');
        const contentEl = document.getElementById('blob-content');
        const dlBtn = document.getElementById('blob-dl-btn');
        if (!titleEl || !contentEl || !dlBtn) return;

        const scroller = document.getElementById('tbl-scroller');
        const curDb = scroller?.dataset.db || new URLSearchParams(location.search).get('db') || '';
        const curTbl = scroller?.dataset.table || new URLSearchParams(location.search).get('table') || '';

        let targetUrl = `?db=${encodeURIComponent(curDb)}&tab=blob_raw&table=${encodeURIComponent(curTbl)}&col=${encodeURIComponent(colName)}&key=${encodeURIComponent(rowKey)}`;

        if (isDirect) {
          const rowItem = window.vRows?.find(r => r[0] === rowKey);
          const colIdx = window.vCols?.indexOf(colName);
          if (rowItem && colIdx !== -1) {
            const raw = rowItem[1][colIdx];
            if (raw && typeof raw === 'object' && raw.uri) {
              targetUrl = raw.uri;
            } else if (typeof raw === 'string') {
              const tr = raw.trim();
              if (tr.startsWith('data:image/')) {
                targetUrl = tr;
              } else if (tr.length > 60 && /^(iVBORw0KGgo|\/9j\/|R0lGOD|UklGR)/.test(tr)) {
                let m = 'image/png';
                if (tr.startsWith('/9j/')) m = 'image/jpeg';
                else if (tr.startsWith('R0lGOD')) m = 'image/gif';
                else if (tr.startsWith('UklGR')) m = 'image/webp';
                targetUrl = `data:${m};base64,${tr}`;
              }
            }
          }
        }

        titleEl.textContent = `${colName} (${(rawLen / 1024).toFixed(1)} KB)`;
        dlBtn.href = isDirect ? targetUrl : `${targetUrl}&dl=1`;

        if (isImg) {
          contentEl.innerHTML = `<div style="text-align:center;padding:1rem;background:var(--erd-bg)"><img src="${targetUrl}" style="max-width:100%;max-height:42dvh;border-radius:8px" alt="Preview"></div>`;
        } else {
          let hexDump = '';
          for (let i = 0; i < (hexSample || '').length; i += 32) {
            const offset = (i / 2).toString(16).padStart(6, '0');
            const chunk = hexSample.substr(i, 32);
            const spaced = chunk.match(/.{1,2}/g)?.join(' ') || '';
            hexDump += `${offset}: ${spaced}\n`;
          }
          contentEl.innerHTML = `
            <div class="muted" style="margin-bottom:.5rem">Hex inspection (initial bytes):</div>
            <pre class="code" style="font-size:.76rem;line-height:1.4">${hexDump || 'Empty payload'}</pre>
          `;
        }
        openBlobSheet();
      }
      ...
      function openEditRowByIndex(idx) {
        if (!window.vRows || !window.vRows[idx]) return;
        const [key, vals] = window.vRows[idx];
        const editPayload = { __rowid__: key };
        window.vCols.forEach((col, cIdx) => {
          const v = vals[cIdx];
          editPayload[col] = (v && typeof v === 'object' && v.__b) ? `[BLOB ${v.len} bytes]` : (v ?? '');
        });
        openEditRow(editPayload);
      }

      window.vRows = [];
      window.vCols = [];
      window.vSelected = new Set();
      let scrollRaf = null;
      const V_ROW_H = 44;
      ...
      function initVirtualTable() {
        ...
        const colsEl = document.getElementById('tbl-cols-json');
        const dataEl = document.getElementById('tbl-raw-json');
        if (!colsEl || !dataEl) return;
        try {
          window.vCols = JSON.parse(colsEl.textContent || '[]');
          window.vRows = JSON.parse(dataEl.textContent || '[]');
        } catch (e) {
          window.vCols = [];
          window.vRows = [];
        }
        window.vSelected.clear();
        renderTableSlice();
      }

      function handleTableScroll() {
        if (scrollRaf) cancelAnimationFrame(scrollRaf);
        scrollRaf = requestAnimationFrame(renderTableSlice);
      }

      function renderTableSlice() {
        const scroller = document.getElementById('tbl-scroller');
        const vbody = document.getElementById('tbl-vbody');
        const padTop = document.getElementById('v-pad-top');
        const padBottom = document.getElementById('v-pad-bottom');
        if (!scroller || !vbody || !padTop || !padBottom || !window.vRows) return;

        const total = window.vRows.length;
        if (total === 0) {
          padTop.style.height = '0px';
          padBottom.style.height = '0px';
          while (vbody.children.length > 2) {
            vbody.removeChild(vbody.children[1]);
          }
          return;
        }

        const curDb = new URLSearchParams(location.search).get('db') || '';
        const curTbl = new URLSearchParams(location.search).get('table') || '';
        const scrollerDb = scroller.dataset.db || curDb;
        const scrollerTbl = scroller.dataset.table || curTbl;
        const canManage = Boolean(document.getElementById('chk-all'));

        const scrollTop = scroller.scrollTop || 0;
        const viewH = Math.max(scroller.clientHeight || 0, window.innerHeight || 600);
        const startIndex = Math.max(0, Math.min(total - 1, Math.floor(scrollTop / V_ROW_H) - 4));
        const endIndex = Math.min(total, Math.ceil((scrollTop + viewH) / V_ROW_H) + 6);

        padTop.style.height = (startIndex * V_ROW_H) + 'px';
        padBottom.style.height = (Math.max(0, total - endIndex) * V_ROW_H) + 'px';

        const frag = document.createDocumentFragment();

        for (let i = startIndex; i < endIndex; i++) {
          const rowItem = window.vRows[i];
          if (!rowItem) continue;
          const [key, vals] = rowItem;
          const tr = document.createElement('tr');
          const isChecked = window.vSelected.has(key) ? 'checked' : '';

          let rowHtml = '';
          if (canManage) {
            rowHtml += `
              <td style="width:40px;min-width:40px;max-width:40px;text-align:center">
                <input type="checkbox" class="row-chk" value="${key}" ${isChecked} onchange="toggleRowSelect('${key}', this.checked)">
              </td>
              <td class="row gap-sm" style="padding:0 .75rem;width:74px;min-width:74px;max-width:74px">
                <button type="button" class="icon-btn" onclick="openEditRowByIndex(${i})" title="Edit">✎</button>
                <form method="post" action="?db=${encodeURIComponent(scrollerDb)}&tab=browse&table=${encodeURIComponent(scrollerTbl)}" onsubmit="return confirm('Delete this record?');" style="display:inline">
          ...
          window.vCols.forEach((col, idx) => {
            const val = vals ? vals[idx] : null;
            if (val === null || val === undefined) {
              rowHtml += `<td><span class="pill-null">NULL</span></td>`;
            } else if (typeof val === 'object' && val.__b) {
              const isDirect = Boolean(val.uri);
              const streamUrl = isDirect ? val.uri : `?db=${encodeURIComponent(scrollerDb)}&tab=blob_raw&table=${encodeURIComponent(scrollerTbl)}&col=${encodeURIComponent(col)}&key=${encodeURIComponent(key)}`;
              if (val.img) {
                rowHtml += `<td><img src="${streamUrl}" class="blob-thumb" loading="lazy" onclick="viewBlobDetail('${col}', '${key}', true, ${val.len || 0}, '${val.hex || ''}', ${isDirect})" alt="img"></td>`;
              } else {
                rowHtml += `<td><button type="button" class="chip" style="padding:1px 6px;font-size:.68rem" onclick="viewBlobDetail('${col}', '${key}', false, ${val.len || 0}, '${val.hex || ''}', false)">BLOB ${((val.len || 0)/1024).toFixed(1)}KB</button></td>`;
              }
            } else {
              const s = String(val);
              const tr = s.trim();
              const isB64Img = tr.startsWith('data:image/') || (tr.length > 60 && /^(iVBORw0KGgo|\/9j\/|R0lGOD|UklGR)/.test(tr));
              const isRawB64 = tr.length > 80 && /^[A-Za-z0-9+/=\s]+$/.test(tr);
              if (isB64Img) {
                let imgUri = tr;
                if (!tr.startsWith('data:image/')) {
                  let m = 'image/png';
                  ...
                  imgUri = `data:${m};base64,${tr}`;
                }
                rowHtml += `<td><img src="${imgUri}" class="blob-thumb" loading="lazy" onclick="viewBlobDetail('${col}', '${key}', true, ${s.length}, '', true)" alt="img"></td>`;
              } else if (isRawB64) {
                rowHtml += `<td><button type="button" class="chip" style="padding:1px 6px;font-size:.68rem" onclick="viewBlobDetail('${col}', '${key}', false, ${s.length}, '', true)">BLOB ${(s.length/1024).toFixed(1)}KB</button></td>`;
              } else {
                const disp = s.length > 50 ? s.slice(0, 50) + '…' : s;
                rowHtml += `<td><span title="${s.replace(/"/g, '&quot;')}">${disp.replace(/</g, '&lt;')}</span></td>`;
              }
            }
          });
        ...
        vbody.insertBefore(frag, padBottom);
      }

      function toggleRowSelect(key, isChecked) {
        if (isChecked) {
          window.vSelected.add(key);
        } else {
          window.vSelected.delete(key);
        }
        updateBulkBar();
      }

      function toggleSelectAll(master) {
        if (!window.vRows) return;
        if (master.checked) {
          window.vRows.forEach(r => window.vSelected.add(r[0]));
        } else {
          window.vSelected.clear();
        }
        handleTableScroll();
      ...
      function updateBulkBar() {
        ...
        const size = window.vSelected.size;
        if (size > 0) {
          bar.style.display = 'flex';
          count.textContent = `${size} record(s) selected`;
          ids.value = Array.from(window.vSelected).join('|');
        } else {
      ...
      function changeRowLimit(newLimit) {
      ...
      document.addEventListener('DOMContentLoaded', () => {
        ...
        if (tab === 'browse') initVirtualTable();
      });
    </script>
  </body>
</html>
